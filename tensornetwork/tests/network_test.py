...
def test_contract_trace_different_nodes_value_error(backend):
  a = tn.Node(np.array([2.]), backend=backend)
  b = tn.Node(np.array([2.]), backend=backend)
  e = tn.connect(a[0], b[0])
  with pytest.raises(ValueError):
    tn._contract_trace(e)
...
def test_contract_copy_node(backend):
  ...
  cn = tn.CopyNode(rank=4, dimension=3)
  tn.connect(a[0], cn[0])
  tn.connect(b[0], cn[1])
  tn.connect(c[0], cn[2])
  tn.connect(d[0], cn[3])
...
def test_outer_product_final_nodes(backend):
  ...
  nodes = [tn.Node(np.ones(i), backend=backend) for i in range(1, 5)]
  edges = []
  for i in range(4):
    edges.append(nodes[i][0])
  final_node = tn.outer_product_final_nodes(nodes, edges)
...
def test_outer_product_final_nodes_not_contracted_value_error(backend):
  ...
  e = tn.connect(a[0], b[0])
  with pytest.raises(ValueError):
    tn.outer_product_final_nodes([a, b], [e])
...
def test_get_parallel_edge(backend):
  ...
  sorted_edges = sorted(list(edges))[0]
  assert tn.get_parallel_edges(sorted_edges) == edges
