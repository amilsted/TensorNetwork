...
def ncon(tensors: Sequence[Tensor],
  ...
  while con_edges:
    ...
    edges_to_contract = network.get_shared_edges(*nodes_to_contract)
  ...
  res_node = network.outer_product_final_nodes(nodes, out_edges)
  ...
  for i, (tensor, edge_lbls) in enumerate(zip(tensors, network_structure)):
    ...
    for (axis_num, edge_lbl) in enumerate(edge_lbls):
      if edge_lbl not in edges:
        ...
        e = network.connect(edges[edge_lbl], node[axis_num], name=str(edge_lbl))
        edges[edge_lbl] = e
  return nodes, edges
