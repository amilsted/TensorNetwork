...
def add_fft(inputs: List[tensornetwork.Edge]) -> List[tensornetwork.Edge]:
  ...
  Args:
    inputs: The edges on which to append the FFT network (input to the FFT).
  ...
  def inline_stitch(targets: List[int], tensor: np.ndarray, name: str):
    ...
    op_node = tensornetwork.Node(tensor, name)
    for k, t in enumerate(targets):
      incoming_state = state[t]
      receiving_port = op_node[k]
      output_port = op_node[k + len(targets)]
      tensornetwork.connect(incoming_state, receiving_port)
      state[t] = output_port
