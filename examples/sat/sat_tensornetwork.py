...
def sat_tn(clauses: List[Tuple[int, int, int]]
  ...
  """Create a 3SAT tensor network of the given 3SAT clauses.
    ...
    assignment (False, True, True) satisfies all clauses.
  ...
  Returns:
    nodes: Nodes comprising the 3SAT tensor network.
  ...
  for _ in range(num_vars):
    new_node = tensornetwork.Node(
      np.ones(2, dtype=np.int32), backend="tensorflow")
  ...
  for clause in clauses:
    ...
    clause_tensor[(-np.sign(a) + 1) // 2, (-np.sign(b) + 1) // 2,
                  (-np.sign(c) + 1) // 2] = 0
    clause_node = tensornetwork.Node(clause_tensor, backend="tensorflow")
    nodes.append(clause_node)

    # Connect the variable to the clause through a copy tensor.
    for i, var in enumerate(clause):
      copy_tensor_node = tensornetwork.Node(
        _build_copy_tensor(), backend="tensorflow")
      nodes.append(copy_tensor_node)
      tensornetwork.connect(clause_node[i], copy_tensor_node[0])
      tensornetwork.connect(var_edges[abs(var) - 1], copy_tensor_node[1])
      var_edges[abs(var) - 1] = copy_tensor_node[2]

  return nodes, var_edges


def sat_count_tn(
    clauses: List[Tuple[int, int, int]]) -> List[tensornetwork.Node]:
  """Create a 3SAT Count tensor network.
  ...
  Returns:
    nodes: The Nodes comprising the tensor network for the norm of the 3SAT
      tensor network generated by the given clauses.
  """
  nodes1, var_edges1 = sat_tn(clauses)
  nodes2, var_edges2 = sat_tn(clauses)
  for edge1, edge2 in zip(var_edges1, var_edges2):
    tensornetwork.connect(edge1, edge2)
